(function () {
  ...
  const externalSources = item.externalSources?.length ? `<div class="project-sources"><strong>Thông tin tham khảo</strong>${item.externalSources.map(source => `<a href="${source.url}" target="_blank" rel="noopener noreferrer">${source.label} <span aria-hidden="true">↗</span></a>`).join('')}</div>` : '';
  root.innerHTML = `<article>
    <header class="detail-hero"><div class="container"><span class="eyebrow">${type === 'project' ? 'Dự án' : 'Bài viết'}</span><h1>${item.title}</h1><div class="detail-meta"><span>${item.category}</span><time datetime="${item.date}">${item.year || window.formatSoynDate(item.date)}</time>${item.demo ? '<span class="badge">Nội dung mẫu</span>' : ''}</div>${factRow}</div></header>
    ...
    <div class="container article-layout"><aside class="article-aside"><strong>${type === 'project' ? 'Soyn Project' : 'Soyn Stories'}</strong><p>${item.excerpt}</p></aside><div class="article"><div class="notice">${item.demo ? 'Đây là nội dung minh họa. Hãy thay bằng dữ liệu, hình ảnh và URL bài đăng thật trong tệp assets/js/content.js.' : 'Dự án thực tế do Soyn triển khai.'}</div>${articleSections}<div class="source-box"><strong>Nguồn nội dung</strong><p>Thông tin dự án và hình ảnh do Soyn cung cấp. Thông tin về khách sạn được đối chiếu từ nguồn chính thức.</p>${externalSources}<a class="btn btn-light" href="${item.facebookUrl}" target="_blank" rel="noopener noreferrer">Xem Fanpage Soyn <span aria-hidden="true">↗</span></a></div></div></div>${gallery}
  </article>`;
})();
