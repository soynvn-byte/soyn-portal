  ...
  function render(category = 'Tất cả') {
    ...
    root.innerHTML = visible.map(item => `<article class="card">
      <a href="detail.html?type=${type === 'projects' ? 'project' : 'post'}&slug=${encodeURIComponent(item.slug)}">
        <div class="card-media"><div class="art ${item.art}" role="img" aria-label="Hình minh họa cho ${item.title}"></div>${item.demo ? '<span class="badge badge-overlay">Nội dung mẫu</span>' : ''}</div>
        <div class="card-body"><div class="card-meta"><span>${item.category}</span><time>${window.formatSoynDate(item.date)}</time></div><h3>${item.title}</h3><p>${item.excerpt}</p><span class="card-link">${type === 'projects' ? 'Xem dự án' : 'Đọc bài viết'} <span aria-hidden="true">↗</span></span></div>
      </a>
