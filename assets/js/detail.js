(function () {
  ...
  root.innerHTML = `<article>
    ...
    <div class="container article-layout"><aside class="article-aside"><strong>${type === 'project' ? 'Soyn Project' : 'Soyn Stories'}</strong><p>${item.excerpt}</p></aside><div class="article"><div class="notice">${item.demo ? 'Đây là nội dung minh họa. Hãy thay bằng dữ liệu, hình ảnh và URL bài đăng thật trong tệp assets/js/content.js.' : 'Dự án thực tế do Soyn triển khai.'}</div>${articleSections}${type === 'post' ? `<div class="source-box"><strong>Nguồn nội dung</strong><p>Bài viết được phát triển từ nội dung do Soyn chia sẻ.</p><a class="btn btn-light" href="${item.facebookUrl}" target="_blank" rel="noopener noreferrer">Xem trên Fanpage Soyn <span aria-hidden="true">↗</span></a></div>` : ''}</div></div>${gallery}
  </article>`;
})();
